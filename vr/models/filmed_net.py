...
class FiLMedNet(nn.Module):
    def __init__(self, vocab, feature_dim=(1024, 14, 14),
        ...
        for fn_num in range(self.num_modules):
            ...
            use_film_cbn = True
            if fn_num+1 > self.final_resblock_with_cbn:  # plus 1 for convenience
                use_film_cbn = False
...
class FiLMedResBlock(nn.Module):
    def __init__(self, in_dim, out_dim=None, with_residual=True, with_batchnorm=True,
                 ...
                 num_layers=1, condition_method='bn-film', with_cbn=1, use_film_cbn=True, debug_every=float('inf')):
        if out_dim is None:
            out_dim = in_dim
